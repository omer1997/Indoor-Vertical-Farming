...
class AtlasI2C:
    ...
    def read(self, num_of_bytes=31):
        ...
        if is_valid:
            ...
            result = str(self.get_device_info()) + ": " +  str(''.join(char_list))
            
            if len(char_list) != 0 and char_list[0] != '?':
                timesta= datetime.datetime.now()
                print('Timestamp : %s' % timesta)
               # result = str(self.get_device_info()) + ": " +  str(''.join(char_list))
                with open('data.csv', 'a') as csvfile:
                    writer = csv.writer(csvfile)
                    #spamwriter = csv.writer(csvfile, delimiter=' ',quotechar='|', quoting=csv.QUOTE_MINIMAL)
                    writer.writerow([timesta] +  [result])
